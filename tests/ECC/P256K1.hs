...
import Crypto.PubKey.ECC.ECDSA (PrivateKey(..), Signature, signDigestWith)
import Crypto.PubKey.ECC.Types (getCurveByName, CurveName(SEC_p256k1))
import Crypto.Number.Serialize (os2ip)
import Crypto.Hash.Algorithms (SHA256(..))
import Crypto.PubKey.ECC.P256K1 (parseDer, rfc6979, Scalar, pointDh, Point, pointFromBinary, secretFromInteger)
...
pri :: Scalar
pri = secretFromInteger $ os2ip $ B.replicate 32 0x12
pub :: Point
