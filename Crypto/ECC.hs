...
instance EllipticCurveDH Curve_P256R1 where
    ecdh s p = SharedSecret $ i2ospOf_ 32 x -- FIXME: 32 is correct?
      where
        (x, _) = P256.pointToIntegers $ P256.pointMul (unP256Scalar s) (unP256Point p)
...
instance EllipticCurveDH Curve_P521R1 where
    ecdh s p = SharedSecret $ i2ospOf_ ((nbBits + 7) `div` 8) x
      where
        H.Point x _ = H.pointMul (H.getCurveByName H.SEC_p521r1) (unP521Scalar s) (unP521Point p)
        nbBits    = H.curveSizeBits $ H.getCurveByName H.SEC_p521r1 -- FIXME: constant?
